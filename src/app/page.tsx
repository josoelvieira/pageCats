...
export default function Home() {
  ...
  const racas = data?.filter((raca) =>
    raca.name.toLocaleLowerCase().includes(search)
  );
  ...
  function geraDot(passo: number) {
    const dots = Array.from({ length: passo }, (_, index) => (
      <span
        key={index}
        className="w-3 h-3 m-1 bg-slate-800 rounded-full"
      ></span>
    ));
  ...
  return (
    <>
      <header className="flex justify-between bg-slate-400 mb-4 p-2 text-black items-center">
        ...
        <div>
          <label className="font-bold text-lg mx-4">Busca por racas:</label>
          <input
            className="p-2"
            type="text"
            value={search}
      ...
      <main className=" w-full h-full">
        <ul className="grid lg:grid-cols-3 md:grid-cols-2 gap-6 m-auto w-4/5">
          {racas?.map((repo) => (
            ...
            >
              <h3 className=" text-lg my-4 font-bold border-b-4">
                {repo.name}
              </h3>
              <Image
                className=" rounded-lg"
                src="https://cdn2.thecatapi.com/images/LSaDk6OjY.jpg"
                width={400}
                height={300}
                alt=""
              />
